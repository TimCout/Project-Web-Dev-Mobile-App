...
export default function Football() {
  ...
  const availableMatches = () => matches()?.filter(match => match.registeredPlayer < match.neededPlayer);
  ...
  return (
    <main class="text-center mx-auto text-gray-700 p-4">
      ...
      {availableMatches()?.length === 0 && (
        <p class="text-gray-500 text-xl mt-8">Aucun match disponible pour le moment</p>
      )}
          ...
          {match => (
            <li class="flex justify-between items-center p-3 my-2 border rounded-lg bg-white shadow-sm">
              <div class="text-sky-700 text-left">
                <span class="font-bold">{moment(match.date).format('LLLL')}</span> -
